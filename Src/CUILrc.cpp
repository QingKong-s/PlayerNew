...
LRESULT CUILrc::OnEvent(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
        ...
        if (eck::PtInRect(rcThumb, pt))
        {
            ...
            SetCapture();
            m_ScrollView.OnLButtonDown(pt.y);
        }
        else
        {
            int idx = HitTest(pt);
    ...
    case WM_SIZE:
    {
        m_ScrollView.SetViewSize(GetViewHeight());
    }
    break;
    ...
    case WM_CREATE:
    {
        ...
        m_ScrollView.SetMinThumbSize(GetBk()->GetDpiSize().cxyMinSBThumb);
